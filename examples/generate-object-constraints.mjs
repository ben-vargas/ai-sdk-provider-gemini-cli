 ...
 * 
 * NOTE: Some Zod constraints (like multipleOf, refine) are not reliably enforced
 * by the model during generation. Use .describe() hints for better results.
 */
...
// NOTE: multipleOf() is not reliably enforced by the model - use .describe() hints instead
    ...
    const { object } = await generateObject({
      ...
      schema: z.object({
        product: z.object({
          ...
          // NOTE: multipleOf(0.01) removed - model may not enforce decimal precision
          price: z.number()
            .positive('Must be positive')
            .min(0.99, 'Minimum price $0.99')
            .max(99999.99, 'Maximum price $99,999.99')
            .describe('Price in dollars with up to 2 decimal places'),
          ...
          // NOTE: multipleOf(0.5) removed - model may not enforce half-star increments
          rating: z.number()
            .min(0, 'Minimum rating 0')
            .max(5, 'Maximum rating 5')
            .describe('Rating from 0 to 5, preferably in half-star increments (0, 0.5, 1, 1.5, ... 5)'),
...
// NOTE: refine() validations run post-generation and may fail if model doesn't follow hints
// NOTE: multipleOf() constraints are not reliably enforced by the model
async function example6_businessRules() {
  ...
  try {
    const { object } = await generateObject({
      ...
      schema: z.object({
        insurance: z.object({
          ...
          coverage: z.object({
            ...
            // NOTE: multipleOf(100) removed - use describe() hint instead
            deductible: z.number()
              .positive()
              .describe('Deductible amount - should be a multiple of $100 (e.g., 500, 1000, 1500)'),
          ...
          premium: z.object({
            ...
            monthly: z.number()
              .positive()
              .describe('Monthly premium amount in dollars'),
          ...
          // NOTE: refine() validation may fail if model doesn't generate percentages totaling 100
          // The describe() hint helps guide the model toward the correct output
          beneficiaries: z.array(z.object({
            ...
            .min(1)
            .max(5)
            .describe('Beneficiaries with percentages that MUST total exactly 100%'),
      ...
      prompt: 'Generate a premium health insurance policy for a 35-year-old non-smoker with 2 beneficiaries. IMPORTANT: The beneficiary percentages must total exactly 100%.',
    });

    console.log('Generated insurance policy with business rules:');
    console.log(JSON.stringify(object, null, 2));
    
    // Manually verify beneficiary percentages
    const total = object.insurance.beneficiaries.reduce((sum, b) => sum + b.percentage, 0);
    if (total !== 100) {
      console.log(`⚠️  Warning: Beneficiary percentages total ${total}%, not 100%`);
    }
  } catch (error) {
...
async function main() {
  ...
  console.log('- ⚠️  multipleOf() and refine() may not be reliably enforced - use describe() hints');
